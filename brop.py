...
class neural_net:
	def __init__(self, dset, d, w):	#takes the training set with the number of hidden nodes
		...
		self.encodings = dset.onehot_encoding()
		self.decodings = dset.onehot_decoding()
		self.init_weights()
		#print self.encodings
		#print self.decodings
		self.testf = arff_file("optdigits_test.arff")
	...
	def train(self,iterations,learning_rate=0.1):
		...
		for i in range(0,iterations):	#loop "iterations" times
			...
			mistakes.append(self.test(self.testf.get_data(),self.testf.get_labels()))
		#print "the weights!"
		#print self.weights
		return mistakes
...
if exp2 == "y":
	...
	plt.figure()
	plt.title("Error rate over iterations on test data in d=3 networks of different widths")
	plt.xlabel("iterations")
	plt.ylabel("percent errors")
	plt.savefig("iter_error_test.png")
	print iter_error
	print train_error
	print test_error
...
if exp3 == "y":
	...
	for r in itererror2:
		plt.plot(range(1,201),r)
	plt.title("error over iterations on test data on w=10 networks of different depths")
	plt.xlabel("iterations")
	plt.ylabel("percent error")
	plt.savefig("itererror_test_exp3.png")
	#print trainerror2
	#print testerror2
